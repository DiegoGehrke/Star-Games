...
object ErrorToastUtils {

    fun showCustomErrorToast(context: Context, message: String, duration: Int, window: Window) {
        ...
        toastBackground.setImageResource(R.drawable.toast_error_background)
        toastText.text = message
        val textColor = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            context.getColor(R.color.red_color_to_error_toast_txt)
        } else {
            ContextCompat.getColor(context, R.color.red_color_to_error_toast_txt)
        }
